...
def train(result_filename, log_filename, write_mode, isFold, lr_list, Dropout_list, wd_list, no_folds = 5, is_split=True):
    ...
    data_dir = "data3/KEGG_Pathway_Image"
    annotation_file = 'annotations/Cancer_annotations_mts3.csv'
    ...
    dataset = CancerDataset(annotation_file,
                            cancer_type, data_dir, transform=normalize)
    ...
    if not is_split:
        ...
        train_indices, test_indices = load_new_split_v2()
...
def main():
    ...
    result_file = "results/ZaSN3_new_cancer.csv"
    log_file = "logs/ZaSN3_new_cancer.txt"
    isFold = True


    write_mode = "w"
    for i in range(3):
        train(result_file, log_file, write_mode, isFold, lr_list, Dropout_list, wd_list)
        write_mode = "a"
